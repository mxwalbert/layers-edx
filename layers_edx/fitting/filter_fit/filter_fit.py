...
class FilterFit(LinearFit):
    @staticmethod
    def fill_data(
        data: npt.NDArray[np.floating], thresh: float = float("inf")
    ) -> npt.NDArray[np.floating]:
        """
        Fills NaN values and values which are below the `thresh` with
    ...
    def __init__(
        ...
        self._const_ff = TopHatFilter(
            detector.calibration.model.fwhm_at_mn_ka, detector.calibration.channel_width
        )
    ...
    def _assign_parameters(self) -> None:
        ...
        for roi, reference in self.references.items():
            ...
            self._parameters.append(
                FilterParameter(filtered, self.naive, self.model_thresh)
            )

    def filter_reference(
        self, reference: BaseSpectrum, roi: RegionOfInterest, element: Element
    ) -> FilteredSpectrum:
        bci = reference.background_corrected_integral(
            ToSI.ev(roi.low_energy), ToSI.ev(roi.high_energy)
        )
        if (bci[0] / bci[1]) > 3.0:
    ...
    def features_and_targets(
        self, unknown: BaseSpectrum, selected: list[FilterParameter]
