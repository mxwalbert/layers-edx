...
class ModelFit(LinearFit):
    ...
    def _assign_parameters(self) -> None:
        ...
        for roi, reference in self.references.items():
            ...
            intensities = {
                xrt: intensity
                for xrt, intensity in reference.model.items()
                if xrt in xrt_set.xrts
            }
            self._parameters.append(ModelParameter(element, xrt_set, intensities))

    def features_and_targets(
        self, unknown: dict[XRayTransition, float], selected: list[ModelParameter]
    ) -> tuple[npt.NDArray[np.floating], npt.NDArray[np.floating]]:
        features = []
        for p in selected:
            features.append(
                [p.intensities[xrt] if xrt in p.intensities else 0.0 for xrt in unknown]
            )
