...
def bound(x: int, lower_inclusive: int, upper_exclusive: int) -> int:
    """Returns `x` bounded so that it exists on the interval [lower_inclusive, upper_exclusive)."""
...
def linear_regression(x: npt.ArrayLike, y: npt.ArrayLike) -> tuple[Callable, float]:
    """Returns the least-squares solution of a 1-dimensional linear equation, i.e., (y_hat, chi2)."""
...
class BaseSpectrum:
    ...
    def __init__(self, properties: SpectrumProperties, data: npt.NDArray[np.floating] = None):
    ...
    def copy(self) -> 'BaseSpectrum':
        """Generates a new instance with a shallow copy of the `properties` and a deep copy of the `data`."""
    ...
    def bound(self, channel: int) -> int:
        """Returns `channel` bounded so that it exists on the interval 0 to `self.channel_count`-1."""
    ...
    def estimate_background(self, mode: str, start: int,
                            min_bins: int = 5, max_bins: int = 50) -> tuple[float, float, int]:
        """Estimate the background starting at the channel `start` heading towards the channel limit, specified by
        `mode` ("low" or "high"). Uses at least `min_bins` but as many as `max_bins` channels to minimize the error
        estimate in the background level."""
        direction = None
        limit = 0
        if mode == 'low':
            direction = -1
        ...
        x, y = zip(*[(channel, self.counts(channel)) for channel in range(start, end + direction, direction)])
        y_hat, best_error = linear_regression(x, y)
        best_channel = end
        best_result = y_hat(start)
        if best_error < 2.0:
            end = self.bound(start + direction * max_bins)
            for channel in range(start + direction * (min_bins + 1), end + direction, direction):
                x.append(channel)
            ...
            for channel in range(start + direction, end + direction, direction):
                ...
                if error < best_error:
                    best_error = error
                    best_result = mean(y)
        return best_result, max((1.0, best_error)), 1 + direction * (best_channel - start)

    def background_corrected_integral(self, e_low: float, e_high: float):
        """Computes the background corrected integral over the specified range of energies (e_low, e_high). The width
        of the regions above and below the peak that is used to estimate the background is adapted dynamically to
        reduce the standard deviation in the background estimate."""
        ...
        if (low[0] + high[0]) != 0.0:
            dev_above = (math.sqrt((low[1] * low[1]) + (high[1] * high[1])) / (low[0] + high[0])) * above
        else:
            dev_above = 1.0
        integral = 0.0
        for channel in range(min_channel, max_channel + 1):
            integral += self.counts(channel)
        return integral - above, math.sqrt(dev_above * dev_above + integral), integral, above
