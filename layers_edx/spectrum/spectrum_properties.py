...
class SpectrumProperties:
    ...
    Attributes:
        detector (TDetector): The detector object on which this spectrum was collected.
            Default is None.
        beam_energy (float): The energy of the electron probe in keV. Default is 20.0.
        probe_current (float): A measurement of the probe current in nanoamperes (nA).
            Default is 1.0.
        live_time (float): The amount of time during which the detector was available to
            process x-ray events in seconds (s). Default is 60.0.
        working_distance (float): The distance between the beam exit and the sample
            surface in millimeters (mm). Default is NaN.
        zero_peak_discriminator (float): The energy of the zero stabilization peak in
            electronvolt (eV). Default is 0.0.
        standard_composition (Optional[Composition]): The composition of the sample as
            determined by an independent method.
        microanalytical_composition (Optional[Composition]): The best microanalytically
    ...
    working_distance: float = (
        10.0 if detector is None else detector.position.optimal_working_distance
    )
    ...
    def sample_distance(self) -> float:
        ...
        """
        return float(
            np.linalg.norm(self.detector.position.coordinates - self.sample_position)
        )
