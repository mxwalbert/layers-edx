...
def read_csv(filename: str,
             value_offset: int = 0, row_offset: int = 0, column_offset: int = 0,
             nan: float | int | str = 0,
             conversion: Callable = lambda x: x,
             dtype: Type[float | int] = float,
             fill_value: Any = None) -> llf | lli:
    ...
    with open(f'{BASE_PATH}/resources/{filename}.csv') as file:
        ...
        for row in csv.reader(file):
            ...
            if row_offset > current:
                continue
            values.append([dtype(nan) if value == '' else conversion(dtype(value)) for value in row[column_offset:]])
    return values
