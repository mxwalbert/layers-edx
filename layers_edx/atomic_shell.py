    ...
    class Chantler2005(Database):
        ...
        def index(cls, shell: AtomicShell) -> int:
            if shell.shell <= AtomicShell.from_name('OV'):
                return shell.shell
            for p_shell in [AtomicShell.from_name(name) for name in ['PI', 'PII', 'PIII']]:
...
class AtomicShell:
    ...
    NAME = [
        ...
        'PI', 'PII', 'PIII', 'PIV', 'PV', 'PVI', 'PVII', 'PVIII', 'PIX', 'PX', 'PXI',
    ...
    def electric_dipole_permitted(cls, shell1: int, shell2: int) -> bool:
        if abs(cls.TOTAL_ANGULAR_MOMENTUM[shell1] - cls.TOTAL_ANGULAR_MOMENTUM[shell2]) > 1.0:
            return False
        return abs(cls.ORBITAL_ANGULAR_MOMENTUM[shell1] - cls.ORBITAL_ANGULAR_MOMENTUM[shell2]) == 1

    @classmethod
    def electric_quadrupole_permitted(cls, shell1: int, shell2: int) -> bool:
        if abs(cls.TOTAL_ANGULAR_MOMENTUM[shell1] - cls.TOTAL_ANGULAR_MOMENTUM[shell2]) > 2.0:
            return False
        if cls.TOTAL_ANGULAR_MOMENTUM[shell1] == 0.5 and cls.TOTAL_ANGULAR_MOMENTUM[shell2] == 0.5:
            return False
        delta_l = abs(cls.ORBITAL_ANGULAR_MOMENTUM[shell1] - cls.ORBITAL_ANGULAR_MOMENTUM[shell2])
    ...
    def energy(self) -> float:
        return self.edge_energy + self.element.ionization_energy if self.ground_state_occupancy else 0.0
