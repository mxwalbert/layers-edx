...
class QuantifyUsingStandards(ABC):
    def __init__(
        ...
        self._stripped_elements = set.union(
            *[standard.stripped_elements for standard in standards.values()]
        )
        self._user_references = {} if user_references is None else user_references
        self._culling_strategy = (
            CullByVariance(0.0) if culling_strategy is None else culling_strategy
        )
    ...
    def assign_reference(self, roi: RegionOfInterest) -> TReferenceMaterial:
        ...
        if element in self.standards and self.standards[
            element
        ].is_suitable_as_reference(roi):
            return self.create_reference(self.standards[element])
        rm = None
        best_sn = 0.0
        for standard in self.standards.values():
            if (
                element in standard.composition.elements
                and standard.is_suitable_as_reference(roi)
            ):
    ...
    def compute_reference_scale(self, roi: RegionOfInterest) -> float:
        """
        Computes the scaling factor between the reference and the standard
        ...
        reference_requirements = (
            standard.required_references[roi]
            if roi in standard.required_references
            else set()
        )
    ...
    def references(self) -> dict[RegionOfInterest, TReferenceMaterial]:
        """
        Returns a map of ROI to the ReferenceMaterial which is available to act as a
        reference for this ROI. Does not return ROIs for which a ReferenceMaterial has
        not been defined. If there is a user specified reference, then this will be
        used. If a standard can act as a reference then this standard is suggested so
        long as the estimated signal-to-noise is adequate.
        """
        return self._references

    @property
    def reference_scales(self) -> dict[RegionOfInterest, float]:
        """Returns the scale of the reference relative to the standard by fitting the
        standard with the appropriate reference (and any other required references) to
        extract the k-ratio of references relative to the standard.
        """
    ...
    def compute(self, unknown: Any) -> KRatioSet:
        ...
        for xrt_set in krs_against_refs.xrt_sets:
            if full_krs.find(xrt_set) is not None:
                ...
                with_stripped.add(
                    xrt_set, krs_against_refs.kratio_from_xrt_set(xrt_set)
                )
