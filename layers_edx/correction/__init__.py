...
class Correction(ABC):
    ...
    def take_off_angle(self) -> float:
        """The angle at which the detector is located. Measured up from the x-y plane in radians (rad)."""
    ...
    def chi(self, xrt: XRayTransition) -> float:
        ...
        return self.mac.compute_composition(self.composition, xrt.energy) / math.sin(self.take_off_angle) # TODO: store for xrt

    @abstractmethod
    def compute_za_correction(self, xrt: XRayTransition) -> float:
        """Computes the ZA or Phi(rho z) part of the ZAF correction. The returned number is not necessarily meaningful
        in-and-of-itself. It may need to be compared with a standard to produce a correction factor."""
    ...
    def compute_zaf_correction(self, xrt: XRayTransition) -> float:
        ...
        f = self.fl.compute(self.composition, xrt, self.beam_energy, self.take_off_angle)
        return za * f
