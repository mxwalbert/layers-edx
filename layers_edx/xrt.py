...
K_FAMILY = [
    'KA1', 'KA2', 'KB1', 'KB2', 'KB3', 'KB4', 'KB5'
]
L_FAMILY = [
    'L3N2', 'L3N3', 'L3O2', 'L3O3', 'L3P1', 'LA1', 'LA2', 'LB15', 'LB2', 'LB5',
    'LB6', 'LB7', 'Ll', 'Ls', 'Lt', 'Lu', 'L2M2', 'L2M5', 'L2N2', 'L2N3',
    'L2N5', 'L2O2', 'L2O3', 'L2P2', 'LB1', 'LB17', 'LG1', 'LG5', 'LG6', 'LG8',
    'Ln', 'Lv', 'L1M1', 'L1N1', 'L1N4', 'L1O1', 'L1O4', 'LB10', 'LB3', 'LB4',
    'LB9', 'LG2', 'LG11', 'LG3', 'LG4', 'LG4p'
]
M_FAMILY = [
    'M1N2', 'M1N3', 'M2M4', 'M2N1', 'M2N4', 'M2O4', 'M3M4', 'M3M5', 'M3N1', 'M3N4',
    'M3O1', 'M3O4', 'M3O5', 'MG', 'M4N3', 'M4O2', 'MB', 'MZ2', 'M5O3', 'MA1',
    'MA2', 'MZ1'
]
N_FAMILY = [
    'N4N6', 'N5N6'
...
SOURCE_SHELL = [AtomicShell.from_name(shell) for shell in [
    'LIII', 'LII', 'MIII', 'NIII', 'MII', 'NV', 'MV',
    'NII', 'NIII', 'OII', 'OIII', 'PI', 'MV', 'MIV', 'NIV', 'NV', 'OIV',
    'NI', 'OI', 'MI', 'MIII', 'MII', 'NVI', 'MII', 'MV', 'NII', 'NIII',
    'NV', 'OII', 'OIII', 'PII', 'MIV', 'MIII', 'NIV', 'NI', 'OIV', 'OI',
    'MI', 'NVI', 'MI', 'NI', 'NIV', 'OI', 'OIV', 'MIV', 'MIII', 'MII',
    'MV', 'NII', 'NV', 'NIII', 'OIII', 'OII',
    'NII', 'NIII', 'MIV', 'NI', 'NIV', 'OIV', 'MIV', 'MV', 'NI', 'NIV',
    'OI', 'OIV', 'OV', 'NV', 'NIII', 'OII', 'NVI', 'NII', 'OIII', 'NVII',
    'NVI', 'NIII', 'NVI', 'NVI'
]]
DESTINATION_SHELL = [AtomicShell.from_name(shell) for shell in [
    'K', 'K', 'K', 'K', 'K', 'K', 'K',
    'LIII', 'LIII', 'LIII', 'LIII', 'LIII', 'LIII', 'LIII', 'LIII', 'LIII', 'LIII',
    'LIII', 'LIII', 'LIII', 'LIII', 'LIII', 'LIII', 'LII', 'LII', 'LII', 'LII',
    'LII', 'LII', 'LII', 'LII', 'LII', 'LII', 'LII', 'LII', 'LII', 'LII',
    'LII', 'LII', 'LI', 'LI', 'LI', 'LI', 'LI', 'LI', 'LI', 'LI',
    'LI', 'LI', 'LI', 'LI', 'LI', 'LI',
    'MI', 'MI', 'MII', 'MII', 'MII', 'MII', 'MIII', 'MIII', 'MIII', 'MIII',
    'MIII', 'MIII', 'MIII', 'MIII', 'MIV', 'MIV', 'MIV', 'MIV', 'MV', 'MV',
    'MV', 'MV', 'NIV', 'NV'
]]
...
class XRayTransition:
    ...
    def _source_shell(cls, element: Element, transition: int) -> int:
        ...
        if atomic_number in [Element.from_name('Li'), Element.from_name('Be')]:
        ...
        elif atomic_number in [Element.from_name('Sc'), Element.from_name('Ti'), Element.from_name('V')]:
        ...
        elif atomic_number in [Element.from_name('Tc'), Element.from_name('Ru'), Element.from_name('Th'),
                               Element.from_name('Pd'), Element.from_name('Ag')]:
            ...
            if result == AtomicShell.from_name('OIII'):
                result = AtomicShell.from_name('OI')
        elif atomic_number in [Element.from_name('Pr'), Element.from_name('Nd'), Element.from_name('Pm'),
                               Element.from_name('Sm'), Element.from_name('Eu')]:
    ...
    @classmethod
    def get_weight(cls, element: Element, transition: int, normalization: str = 'default') -> float:
        ...
        if normalization == 'default':
            ...
            return LINE_WEIGHT[z][transition] / FAMILY_NORM[z][family_from_transition(transition)]
        elif normalization == 'destination':
            return LINE_WEIGHT[z][transition] / DESTINATION_NORM[z][destination_shell_from_transition(transition)]
        elif normalization == 'klm':
            return LINE_WEIGHT[z][transition] / KLM_NORM[z][family_from_transition(transition)]
    ...
    def __init__(self, element: Element, transition: int | str = None, source: int = None, destination: int = None):
        """Create an object corresponding to a specific x-ray transition in a specific element."""
        if transition is not None:
            self._transition = transition_from_name(transition) if type(transition) == str else transition
            self._source = AtomicShell(element, self._source_shell(element, self._transition))
            self._destination = AtomicShell(element, DESTINATION_SHELL[self._transition])
            ...
            raise ValueError("Either `transition` or `source` & `destination` must be provided!")

    def __eq__(self, other) -> bool:
        if isinstance(other, XRayTransition):
            return (self.transition == other.transition and
                    self.source_shell == other.source_shell and
                    self.destination_shell == other.destination_shell)
        return False
...
class XRayTransitionSet:
    ...
    def __init__(self, element: Element,
                 low_energy: float = 0.0, high_energy: float = float('inf'), min_weight: float = 0.0,
                 populate: bool = True):
        ...
        for transition in range(len(NAME)):
            ...
            if xrt.exists and low_energy <= xrt.energy <= high_energy and min_weight <= xrt.weight('klm'):
                self._xrts.add(xrt)
