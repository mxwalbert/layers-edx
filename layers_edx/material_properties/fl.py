...
class Fluorescence:
    ...
    X-ray fluorescence occurs when a primary characteristic X-ray emitted from one element
    excites another element in the sample, potentially enhancing its X-ray emission.
    """

    @classmethod
    def primary_exciting_line(cls, element: Element, shell: AtomicShell) -> XRayTransition | None:
        """
        Determines the most effective X-ray transition in an element that can excite the given atomic shell.

        Args:
            element (Element): The element from which the primary X-ray emission originates.
            shell (AtomicShell): The atomic shell of interest in the target material.

        Returns:
            XRayTransition | None: The most suitable exciting X-ray transition, or None if no valid transition is found.
        """
    ...
    class Algorithm(Protocol):
        """
        Protocol defining the interface for computing secondary fluorescence enhancement.
        """
        ...
        @classmethod
        def compute(cls, composition: Composition, primary: XRayTransition, secondary: XRayTransition, beam_energy: float, take_off_angle: float) -> float:
            """
            Method for computing the contribution of a primary X-ray line to the enhancement
            of a secondary X-ray line through fluorescence.

            Args:
                composition (Composition): The sample's elemental composition.
                primary (XRayTransition): The primary exciting X-ray transition.
                secondary (XRayTransition): The secondary X-ray transition being enhanced.
    ...
    class Reed1993(Algorithm):
        ...
        def compute(cls, composition: Composition, primary: XRayTransition, secondary: XRayTransition, beam_energy: float, take_off_angle: float) -> float:
            ...
            if primary.energy < edge_energy:
                return 0.0
            if primary.element not in composition.elements or secondary.element not in composition.elements:
                return 0.0
            c_p = composition.weight_fractions[primary.element]
            mac_ps = FromSI.cm2pg(cls.mac.compute(secondary.element, primary.energy))
            mac_p = FromSI.cm2pg(cls.mac.compute_composition(composition, primary.energy))
            ...
            u = FromSI.cm2pg(cls.mac.compute_composition(composition, secondary.energy)) / (math.sin(take_off_angle) * mac_p)
            ...
            return f * 0.5 * c_p * (mac_ps / mac_p) * ionize_f * fluor_p * (a_s / a_p) * ss * ((math.log(1.0 + u) / u) + (math.log(1.0 + v) / v))


    @classmethod
    def compute(cls, composition: Composition, secondary: XRayTransition, beam_energy: float, take_off_angle: float) -> float:
        """
        Computes the total fluorescence enhancement for a given secondary X-ray transition
        across all elements in the target material.
        ...
        Args:
            composition (Composition): The sample's elemental composition.
            secondary (XRayTransition): The X-ray line for which enhancement is calculated.
            beam_energy (float): The electron beam energy (J).
            take_off_angle (float): The detector take-off angle (rad).

        Returns:
            float: The total fluorescence enhancement factor (dimensionless). A return value of 1.0
            indicates no fluorescence enhancement.
        """
        total = 0.0
        for element in composition.elements:
            ...
            for transition in range(len(NAME)):
                xrt = XRayTransition(element, transition)
                if xrt.exists and xrt.family == primary_family and xrt.energy >= secondary_ee and xrt.edge_energy < beam_energy:
                    weight += xrt.weight('family')
            if weight > 0.0:
                total += cls.Reed1993.compute(composition, primary, secondary, beam_energy, take_off_angle) * weight
        return 1.0 + total
