...
class JumpRatio:
    ...
    which quantifies the discontinuity in the mass absorption coefficient at the shell’s ionization energy.
    """
    ...
    class Heinrich1986(Algorithm):
        ...
        def compute(cls, shell: AtomicShell) -> float:
            ...
            return cls.mac.compute(shell.element, edge_energy + cls.DELTA) / cls.mac.compute(shell.element, edge_energy - cls.DELTA)

    @classmethod
    def compute(cls, shell: AtomicShell) -> float:
        """
        Computes the jump ratio for a given atomic shell using the default algorithm (Heinrich 1986).
    ...
    def ionization_fraction(cls, shell: AtomicShell) -> float:
        ...
        Returns:
            float: The dimensionless ionization fraction, defined as (r - 1) / r for r ≥ 1, otherwise 0.
        """
        r = cls.compute(shell)
