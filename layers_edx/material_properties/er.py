...
class ElectronRange:
    """
    Provides methods to compute the range of electrons in a material, based on its composition
    and the incident electron energy.

    The electron range is used for estimating the depth of interaction and X-ray generation,
    and its unit is length * density (m * kg/m^3).
    """
    ...
    class Algorithm(Protocol):
        ...
        def compute(cls, composition: Composition, energy: float) -> float:
            ...
            Args:
                ...
                shell (AtomicShell): The atomic shell for which the electron range is calculated.
                energy (float): The incident electron energy (J).

            Returns:
                float: The electron range, representing the penetration depth of electrons (m * kg/m^3).
            """
            ...
    ...
    class PouchouAndPichoir1991(Algorithm):
        ...
        def compute(cls, composition: Composition, energy: float) -> float:
            ...
            tmp = np.power(j, 1.0 - p) * d * np.power(FromSI.kev(energy), 1.0 + p) / (1.0 + p)
            return ToSI.gpcm2(tmp.sum() / big_m)

    @classmethod
    def compute(cls, composition: Composition, shell: AtomicShell, energy: float) -> float:
        """
        Computes the effective electron range between the incident energy and the ionization energy
        of a given atomic shell.
        ...
        Returns:
            float: The difference in electron range between the two energies (m * kg/m^3).
        """
        algorithm = cls.PouchouAndPichoir1991
        return algorithm.compute(composition, energy) - algorithm.compute(composition, shell.edge_energy)
