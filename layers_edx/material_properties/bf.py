...
class BackscatterFactor:
    """
    Provides methods to compute the X-ray backscatter factor, which corrects for
    electron backscattering effects in quantitative X-ray microanalysis.

    The backscatter factor adjusts the detected X-ray intensity to account for the fact
    that some electrons are reflected back out of the material, reducing the effective
    ionization volume.
    """
        ...
        def compute(
            ...
            Args:
                composition (Composition): The elemental composition of the target
                    material.
                shell (AtomicShell): The atomic shell for which the factor is
                    calculated.
    ...
    class PouchouAndPichoir1991(Algorithm):
        ...
        def compute(
            ...
            q = ((2.0 * w) - 1.0) / (1.0 - w)
            gu = (u0 - 1.0 - ((1.0 - (1.0 / math.pow(u0, 1.0 + q))) / (1.0 + q))) / (
                (2.0 + q) * ju
            )
    ...
    def compute(
        ...
        """
        Computes the backscatter factor for the given composition, atomic shell and
        incident energy.
