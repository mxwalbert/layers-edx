...
class FluorescenceYieldMean:
    ...
    class Bambynek1972(Algorithm):
        ...
        def compute(cls, shell: AtomicShell) -> float:
            return (
                cls.DATA[shell.element.atomic_number][shell.family]
                if shell.family <= 2
                else 0.0
            )
    ...
    class Oz1999(Algorithm):
        DATA: list[float] = [
            ...
            for v in read_csv(
                "Oz1999",
                value_offset=Element.from_name("Cu"),
                row_offset=2,
                fill_value=0,
            )
        ]
    ...
    def compute(cls, shell: AtomicShell) -> float:
        ...
        Currently, the method automatically selects the appropriate algorithm based on
        shell family:
