...
class KRatioSet:
    ...
    def add(self, xrt_set: XRayTransitionSet, kratio: float):
        """Adds the `kratio` to the data dictionary. Overwrites an existing value if the `xrt_set` is already in the
        data dictionary."""
    ...
    def find(self, xrt_set: XRayTransitionSet) -> float:
        """Checks if the `xrt_set` already has an associated k-ratio. Otherwise, checks for a similar
        ``XRayTransitionSet`` to return its value."""
    ...
    def kratio_from_xrt(self, xrt: XRayTransition) -> float:
        """Returns the k-ratio value associated with an ``XRayTransitionSet`` which contains the `xrt`."""
