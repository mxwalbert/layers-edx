...
def compute_resolution(fano: float, noise: float, energy: float) -> float:
    """
    Computes the detector resolution at the specified `energy` (in eV)
    for the specified `fano` factor and `noise`.
    """
    return ENERGY_PER_EH_PAIR * math.sqrt(
        noise**2 + (energy * fano) / ENERGY_PER_EH_PAIR
    )


def noise_from_resolution(fano: float, resolution: float, energy: float) -> float:
    """
    Computes the noise term given the `fano` factor, `resolution` at a
    specified `energy` and the energy (in eV).
    """
    return math.sqrt(
        (resolution / ENERGY_PER_EH_PAIR) ** 2 - (energy * fano) / ENERGY_PER_EH_PAIR
    )


def fwhm_to_gaussian_width(fwhm: float) -> float:
    """
    Converts between full width-half maximum and Gaussian width.
    (i.e. 2*x such that exp(-0.5*(x/a)^2)=1/2).
    """
...
class LineshapeModel(ABC):
    ...
    def scale(self) -> float:
        """
        A scale factor to account for differences in geometric efficiencies
        not correctly accounted for by geometric factors.
        """
        return 1.0
    ...
    def compute(self, energy: float, center: float) -> float:
        """
        Computes the intensity at the specified `energy` (in eV) for a peak at
        `center` (in eV). The integral over all energies is normalized to 1.0.
        """

    @abstractmethod
    def left_width(self, energy: float, fraction: float) -> float:
        """
        The inverse of compute on the left (lower energy side of the peak.)
        At what `energy` (in eV) is the peak intensity diminished
        to a `fraction` of its peak value.
        """

    @abstractmethod
    def right_width(self, energy: float, fraction: float) -> float:
        """
        The inverse of compute on the right (higher energy side of the peak.)
        At what `energy` (in eV) is the peak intensity diminished to a `fraction`
        of its peak value.
        """
...
class FanoSiLiLineshape(LineshapeModel):
    def __init__(
        ...
        if fwhm_at_mn_ka is not None:
            self.fano = 0.122
            self.noise = noise_from_resolution(
                self.fano, fwhm_to_gaussian_width(fwhm_at_mn_ka), EMnKa
            )
        else:
            self.fano = fano
            self.noise = noise
